...
class Chat:
    __sesion: Autorizacion = None
    __conversacion: Conversacion
    def __init__(self):
        self.__sesion = Autorizacion()
        self.__conversacion = None
    ...
    def obtener_ultimos_mensajes(self):
        usuario_actual = self.__sesion.usuario_actual
        if usuario_actual is None:
            raise ValueError("No hay un usuario autenticado.")
        mensajes = MensajeModel.obtener_ultimos_mensajes(self.__sesion.usuario_actual.id)
        return mensajes
    ...
    def __iniciar_conversacion(self):
        if self.__sesion.usuario_actual is None:
            raise ValueError("No hay un usuario autenticado.")
        if self.__conversacion is None:
            try:
                self.__conversacion = Conversacion(id_usuario=self.__sesion.usuario_actual.id)
